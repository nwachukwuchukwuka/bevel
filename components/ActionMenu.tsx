...
export const ActionMenu = ({ visible, onClose, onOpenTemplates }: ActionMenuProps) => {
    const insets = useSafeAreaInsets();
    const router = useRouter();
    ...
    const handlePress = (item: any) => {
        onClose();
        setTimeout(() => {
            if (item.label === "Describe food") router.push('/describe-food');
            else if (item.label === "Import food") router.push('/import-food');
            else if (item.label === "Capture food") router.push('/capture-food');
            else if (item.label === "Scan food") router.push('/scan-barcode');
            else if (item.label === "Ask Bevel") router.push('/ask-bevel');
            else if (item.label === "Search food") router.push('/search-food');
            else if (item.label === "Generate\ntemplates") router.push('/generate-template/1-experience');
            else if (item.label === "View templates") onOpenTemplates();
            else if (item.label === "Log activity") router.push('/log-activity');
        }, 100);
    };
    ...
    return (
        ...
        >
            ...
            >
                <Pressable onPress={(e) => e.stopPropagation()}>
                    <View className="rounded-[32px] p-6 pb-20" style={{ backgroundColor: '#151E33', borderWidth: 1, borderColor: '#1E293B' }}>
                        <View className="flex-row flex-wrap justify-between gap-y-6">
                            {MENU_ITEMS.map((item, index) => (
                                <TouchableOpacity
                                    key={index}
                                ...
                                >
                                    <View className="mb-2">
                                        {item.type === 'special' ? (
                                            ...
                                            >
                                                <View className="h-14 w-14 rounded-full items-center justify-center" style={{ backgroundColor: 'rgba(59,130,246,0.15)' }}>
                                                    <LinearGradient
                                                        colors={['rgba(59,130,246,0.4)', 'rgba(59,130,246,0.05)']}
                                                        className="absolute top-0 left-0 w-full h-1/2 rounded-t-full"
                                                    />
                                                </View>
                                            </LinearGradient>
                                        ) : (
                                            <View className="h-16 w-16 rounded-full items-center justify-center" style={{ backgroundColor: '#1E293B', borderWidth: 1, borderColor: 'rgba(30,41,59,0.8)' }}>
                                                <Ionicons
                                                    name={getIconName(item.icon)}
                                                    size={26}
                                                    color="#94A3B8"
                                                />
                                                {item.icon === 'image' && (
                                                    <View className="absolute bottom-4 right-4 rounded-full w-3.5 h-3.5 items-center justify-center" style={{ backgroundColor: '#F97316', borderWidth: 1.5, borderColor: '#151E33' }}>
                                    ...
                                    <Text className="text-[11px] font-semibold text-center leading-4" style={{ color: item.type === 'special' ? '#93C5FD' : '#94A3B8' }}>
                                        {item.label}
                                    </Text>
                                </TouchableOpacity>
                            ))}
